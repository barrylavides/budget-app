...
- Name: issue-8-ac.spec.ts >> AC1: /recurring page lists templates with name, category, half, amount, cadence, tag, active toggle
- Location: e2e/issue-8-ac.spec.ts:106:0
...
Locator: locator('[data-testid="template-name-00000000-0000-0000-0000-000000000060"]')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('[data-testid="template-name-00000000-0000-0000-0000-000000000060"]')
...
```ts
  22  |       Prefer: "return=minimal",
  23  |     },
  24  |   });
  25  | }
  26  | 
  27  | async function apiGet(path: string) {
  28  |   const res = await fetch(`${SUPABASE_URL}/rest/v1/${path}`, {
  29  |     headers: {
  30  |       apikey: SERVICE_ROLE_KEY,
  31  |       Authorization: `Bearer ${SERVICE_ROLE_KEY}`,
  ...
  36  | }
  37  | 
  38  | async function apiPost(path: string, body: unknown) {
  39  |   return fetch(`${SUPABASE_URL}/rest/v1/${path}`, {
  40  |     method: "POST",
  41  |     headers: {
  42  |       apikey: SERVICE_ROLE_KEY,
  43  |       Authorization: `Bearer ${SERVICE_ROLE_KEY}`,
  ...
  49  | }
  50  | 
  51  | // Clean up test-created months (keep the seed month 00000000-0000-0000-0000-000000000010)
  52  | async function cleanupTestMonths() {
  53  |   await apiDelete(
